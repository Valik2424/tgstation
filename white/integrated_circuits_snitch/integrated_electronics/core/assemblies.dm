...
/obj/item/electronic_assembly/Initialize()
	...
	for(var/datum/atom_hud/data/diagnostic/diag_hud in GLOB.huds)
		diag_hud.add_to_hud(src) //undefined proc: "add_to_hud" on /datum/atom_hud/data/diagnostic
...
/obj/item/electronic_assembly/Destroy()
	...
	for(var/datum/atom_hud/data/diagnostic/diag_hud in GLOB.huds)
		diag_hud.remove_from_hud(src) // undefined proc: "remove_from_hud" on /datum/atom_hud/data/diagnostic
...
/obj/item/electronic_assembly/pickup(mob/living/user)
	. = ..()
	//update diagnostic hud when picked up, true is used to force the hud to be hidden // бля тут хуйня
...
/obj/item/electronic_assembly/welder_act(mob/living/user, obj/item/I) //изменено
	var/type_to_use
	...
	switch(type_to_use)
		if("repair")
			to_chat(world,"Integrity: [atom_integrity] / [max_integrity]") // Виправлено: obj_integrity -> atom_integrity
			if(atom_integrity < max_integrity) // Виправлено: obj_integrity -> atom_integrity
				atom_integrity = min(atom_integrity + 20,max_integrity) // Виправлено: obj_integrity -> atom_integrity
				to_chat(world,"Integrity: [atom_integrity] / [max_integrity]") // Виправлено: obj_integrity -> atom_integrity
...
/obj/item/electronic_assembly/attackby(obj/item/I, mob/living/user, params)
	...
	if(istype(I, /obj/item/integrated_circuit_old))
		...
		if(try_add_component(I, user))
			...
			for(var/obj/item/integrated_circuit_old/input/S in assembly_components)
				S.attackby_react(I, user) //  Виправлення:  видалено  user.a_intent
			return ..()
...
/obj/item/electronic_assembly/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change = TRUE)  //  Виправлення:  додано  аргументи
	. = ..(old_loc, movement_dir, forced, old_locs, momentum_change)  //  Виправлення:  виклик  батьківської  процедури
	for (var/I in assembly_components)
		var/obj/item/integrated_circuit_old/IC = I
		IC.ext_moved(old_loc, movement_dir)
...
/obj/item/electronic_assembly/wallmount/proc/mount_assembly(turf/on_wall, mob/user)
	...
	var/turf/wall_turf = get_step(T, ndir) // Отримуємо плитку стіни
	if (wall_turf.density) // Перевіряємо щільність
