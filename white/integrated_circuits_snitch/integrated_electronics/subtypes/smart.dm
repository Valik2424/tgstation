...
/obj/item/integrated_circuit_old/smart/advanced_pathfinder/do_work()
	...
	var/list/P = get_path_to(assembly, locate(get_pin_data(IC_INPUT, 1), get_pin_data(IC_INPUT, 2), a_loc.z), max_distance = 300, access = idc.access, exclude=get_turf(get_pin_data_as_type(IC_INPUT, 3, /atom)), simulated_only = FALSE)
...
/obj/item/integrated_circuit_old/input/pAI_connector
	...
	var/mob/living/silicon/pai/installed_pai

/obj/item/integrated_circuit_old/input/pAI_connector/attackby(var/mob/living/silicon/pai/O, var/mob/user)  // Виправлення: змінено тип аргументу на /mob/living/silicon/pai
	if(!istype(O, /mob/living/silicon/pai))  // Виправлення: змінено тип на /mob/living/silicon/pai
		to_chat(user, span_warning("You can't put that inside."))
		return
	if(installed_pai)
	...
	O.remote_control = src  // Виправлення:  видалено  O.pai,  оскільки  O  вже  є  pAI
	set_pin_data(IC_OUTPUT, 1, O)
...
/obj/item/integrated_circuit_old/input/pAI_connector/proc/RemovepAI()
	if(installed_pai)
		...
		set_pin_data(IC_OUTPUT, 1, WEAKREF(null))
		var/mob/M = installed_pai
		M.remote_control = WEAKREF(null)
//pAI changes
