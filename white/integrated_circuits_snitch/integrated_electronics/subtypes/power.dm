...
/obj/item/integrated_circuit_old/power/transmitter/do_work()
	...
	if(cell)
		...
		if(A.Adjacent(B))
			...
			var/list/transmitters_on_turf = list()
			for(var/obj/O in A.contents) // Виправлення 1: використання contents
				if(istype(O, /obj/item/integrated_circuit_old/power/transmitter)) // Виправлення 2: фільтрація списку
					transmitters_on_turf += O
			transfer_amount *= 1 / transmitters_on_turf.len
