		...
		switch(get_pin_data(IC_INPUT, 2))
			...
			if(2)
				if(TR.myseed) //Could be that they're just using it as a de-weeder
					TR.age = 0
					TR.plant_health = 0
					if (TR.plant_status == HYDROTRAY_PLANT_HARVESTABLE)  //  Виправлення:  перевірка  plant_status
						TR.set_plant_status(HYDROTRAY_PLANT_GROWING)  //  Виправлення:  використання  set_plant_status()
					qdel(TR.myseed)
					TR.myseed = null
				TR.weedlevel = 0 //Has a side effect of cleaning up those nasty weeds
				TR.plantdies() //  Виправлення:  використання  plantdies()
			if(3)
				if (!check_target(O))
					activate_pin(2)
					return FALSE
					...
					if (!TR.myseed)
						...
						TR.set_plant_status(HYDROTRAY_PLANT_GROWING)  //  Виправлення:  встановлення  plant_status
						TR.myseed = O
						TR.age = 1
		...
		if(2)
			...
			for(var/I in 1 to mtypes.len)
				...
				if(M)
					...
					if(!mt) //Invalid input
						if(U > 0)
							suc = TRUE  // Немає сенсу переносити, бо mt не визначено
					else
						var/amount_to_transfer = abs(U)
						if (U > 0) // Перенесення з mt до materials
							var/amount_removed = mt.use_amount_mat(amount_to_transfer, M)
							if (amount_removed > 0)
								var/amount_added = materials.insert_amount_mat(amount_removed, M)
								suc = amount_added > 0 // Перевірка, чи було додано хоча б щось
						else // Перенесення з materials до mt
							var/amount_removed = materials.use_amount_mat(amount_to_transfer, M)
							if (amount_removed > 0)
								var/amount_added = mt.insert_amount_mat(amount_removed, M)
								suc = amount_added > 0 // Перевірка, чи було додано хоча б щось
			if(suc)
				AfterMaterialInsert()
				activate_pin(3)
...
/obj/item/integrated_circuit_old/manipulation/inserter/do_work(mob/user)  //  Додано  аргумент  user
	//There shouldn't be any target required to eject all contents
	...
	switch(mode)
		if(1) //  Вставка
			...
			var/datum/storage/STR = container.atom_storage  //  Виправлення  1:  atom_storage
			if(!STR)
				return

			STR.attempt_insert(target_obj, user)  //  Виправлення  2:  attempt_insert()
		else //  Видалення
			var/datum/storage/STR = target_obj.loc.atom_storage  //  Виправлення  1:  atom_storage
			if(!STR)
				return

			if(!container || !istype(container,/obj/item/storage) || !Adjacent(container))
				STR.attempt_remove(target_obj, drop_location())  //  Виправлення  3:  attempt_remove()
			else
				STR.attempt_remove(target_obj, container)  //  Виправлення  3:  attempt_remove()
