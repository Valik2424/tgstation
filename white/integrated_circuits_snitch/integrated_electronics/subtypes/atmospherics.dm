...
/obj/item/integrated_circuit_old/atmospherics/pump/proc/move_gas(datum/gas_mixture/source_air, datum/gas_mixture/target_air)
	...
	if(source_air.return_temperature() < TCMB)
		source_air.temperature = TCMB // Виправлення: прямий доступ до поля
...
/obj/item/integrated_circuit_old/atmospherics/pump/filter/do_work()
	...
	for(var/filtered_gas in removed.gases)
		...
		if(GLOB.meta_gas_info[filtered_gas][META_GAS_NAME] in wanted)
			...
			filtered_out.gases[filtered_gas][MOLES] = removed.gases[filtered_gas][MOLES] // Виправлення
...
/obj/item/integrated_circuit_old/atmospherics/tank/freezer/process()
	...
	air_contents.temperature = max(73.15, air_contents.return_temperature() - (air_contents.return_temperature() - temperature) * heater_coefficient)
...
/obj/item/integrated_circuit_old/atmospherics/tank/freezer/heater/process()
	...
	air_contents.temperature = min(573.15, air_contents.return_temperature() + (temperature - air_contents.return_temperature()) * heater_coefficient)
...
/obj/item/integrated_circuit_old/atmospherics/cooler/process()
	...
	//Cool the gas
	turf_air.temperature = max(243.15, turf_air.return_temperature() - (turf_air.return_temperature() - temperature) * heater_coefficient)
...
/obj/item/integrated_circuit_old/atmospherics/cooler/heater/process()
	...
	//Heat the gas
	turf_air.temperature = min(323.15, turf_air.return_temperature() + (temperature - turf_air.return_temperature()) * heater_coefficient)
