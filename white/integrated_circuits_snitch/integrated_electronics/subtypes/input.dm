...
/obj/item/integrated_circuit_old/input/plant_scanner/do_work()
	...
	if(H in view(get_turf(src))) // Like medbot's analyzer it can be used in range..
		...
		set_pin_data(IC_OUTPUT, 16, H.plant_status == HYDROTRAY_PLANT_HARVESTABLE)  // Виправлення: перевірка на готовність до збору
		set_pin_data(IC_OUTPUT, 17, H.plant_status == HYDROTRAY_PLANT_DEAD)  // Виправлення: перевірка на смерть
...
/obj/item/integrated_circuit_old/input/adjacent_locator/do_work()
	...
	if(valid_things.len)
		...
		activate_pin(3)
	O.push_data()
/* их уже нету и етот код уже не нужен
...
/obj/item/integrated_circuit_old/input/microphone/Hear(message, atom/movable/speaker, message_langs, raw_message, radio_freq, spans, message_mode, list/message_mods = list(), message_range=0) // Виправлення:  додано  аргументи  message_mods  і  message_range
	. = ..()
...
/obj/item/integrated_circuit_old/input/sensor/sense(atom/A, mob/user, prox)
	...
	if(ignore_bags)
		var/STR = A.GetComponent(/datum/component/storage) //помилка failed to resolve path /datum/component/storage
		if(STR)  // Виправлення: перевірка на null
			return FALSE
	set_pin_data(IC_OUTPUT, 1, WEAKREF(A))
	push_data()
...
/obj/item/integrated_circuit_old/input/atmospheric_analyzer/do_work()
	...
	for(var/id in air_contents.gases) // Виправлення 1: Використання air_contents.gases
		var/name = GLOB.meta_gas_info[id][META_GAS_NAME] // Виправлення 2: Використання GLOB.meta_gas_info та META_GAS_NAME
		var/amt = round(air_contents.gases[id][MOLES], 0.001) // Виправлення 3: Використання air_contents.gases[id][MOLES]
		gas_names.Add(name)
		gas_amounts.Add(amt)
